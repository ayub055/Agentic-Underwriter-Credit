...
export default function Narrator({ text, thinking = false, nextAction = null }) {
  ...
  return (
    <div className="mt-5 flex items-start gap-3 rounded-2xl border border-agent-200/70 bg-gradient-to-br from-agent-50 to-white p-4 sm:p-5">
      <AgentAvatar thinking={thinking} />
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="text-[10px] font-semibold uppercase tracking-widest text-agent-600">
          Kotak AI · Underwriting Agent
        </div>
        <p aria-live="polite" className="mt-1 text-sm leading-relaxed text-ink">
          {typed.text}
          {thinking && typed.done && <ThinkingDots />}
        </p>
        {nextAction && typed.done && (
          <div className="mt-2.5 inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-agent-700 ring-1 ring-agent-200 animate-fade-up">
