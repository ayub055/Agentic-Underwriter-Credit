...
export const PHASES = [
  {
    ...
    subPhases: [
      ...
      { id: "kyc_api", label: "KYC API", detail: "EPFO · consent captured", tone: "mock" },
      {
        ...
        tone: addr.review_flag ? "warn" : "placeholder",
      },
    ],
    agents: [
      ...
      ...(addr.reasons ?? []).map((rsn) => ({
        ...
        detail: rsn.label,
        tone: rsn.direction === "positive" ? "ok" : "warn",
      })),
      { actor: "address_agent", action: "pd_nudge", detail: `pd_adjustment ${num(addr.pd_adjustment)} → folded into ML · L0_ADDRESS ${addr.review_flag ? "flag" : "clear"}`, tone: addr.review_flag ? "warn" : "info" },
      { actor: "validator", action: "schema", detail: "intake ✓", tone: "ok" },
    ],
    data: [
      line("loan_amount_req", formatINR(ik.loan_amount_req), "amber", "intake.loan_amount_req"),
      line("tenure_req", `${num(ik.tenure_req)} months`, "amber"),
      ...
      line("address_score", `${num(addr.score)} / 100 · ${addr.band ?? emDash}`, addr.band === "LOW" ? "bad" : "ok", "address.score"),
      line("address_pd_adjustment", num(addr.pd_adjustment), "amber", "address.pd_adjustment"),
...

  {
    ...
    branches: [
      {
        ...
        data: [
          ...
          line("total_exposure", formatINR(rs.total_exposure), "amber", "summary.total_exposure"),
      ...
      },
      {
        ...
        data: [
          ...
          line("existing_emi_debits", `${formatINR(bs.existing_emi_debits)} / mo`, "amber", "summary.existing_emi_debits"),
    ...
    agents: [
      ...
      { actor: "validator", action: "validate_layer2", detail: `${cs.warnings.filter((w) => w.startsWith("layer2")).length} warnings · no fabrication (absent → None)`, tone: "warn" },
...

  {
    ...
    data: [
      ...
      line("foir_existing", num(ml.foir_existing), "amber", "ml.foir_existing"),
    ],
    warnings: [],
...

  {
    ...
    agents: [
      ...
      ...(pol.layers ?? []).map((l) => ({
        ...
        tone: l.passed ? "ok" : "warn",
      })),
      { actor: "policy", action: pol.result, detail: `segment ${pol.approved_segment} · ai_assisted=${pol.ai_assisted_flag}`, tone: pol.result === "APPROVED" ? "ok" : "warn" },
    ],
    data: [
      ...(pol.layers ?? []).map((l) => line(l.layer, l.passed ? "PASS" : "BREACH", l.passed ? "ok" : "bad")),
      line("policy_result", pol.result, pol.result === "APPROVED" ? "ok" : "bad"),
...

  {
    ...
    data: [
      ...
      line("serviceable", bool(dec.serviceable), dec.serviceable ? "ok" : "bad"),
      line("foir_proposed", num(dec.foir_proposed), "bad", "decision.foir_proposed"),
      line("offer_amount", formatINR(dec.offer_amount), "info"),
      line("offer_tenure", `${num(dec.offer_tenure)} months`, "info"),
      line("offer_irr", formatPct(dec.offer_irr == null ? null : dec.offer_irr * 100), "amber", "decision.offer_irr"),
      line("offer_emi", formatEMI(dec.offer_emi), "amber", "decision.offer_emi"),
    ],
...

  {
    ...
    agents: [
      { actor: "tool:stamp_decision", action: cs.outcome, detail: "outcome stamped", tone: cs.outcome === "APPROVED" ? "ok" : "warn" },
    ...
    data: [
      line("DECISION", cs.outcome, cs.outcome === "APPROVED" ? "ok" : "bad"),
