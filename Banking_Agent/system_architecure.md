...
> ⚠️ **Partially superseded by the 2026-06 consolidation restructure.** Much of the
> detail below predates a major simplification. Current ground truth is `CLAUDE.md`.
> Summary of what changed:
> - **One report path** (`tools/bank_report.generate_bank_report` → `customer_report_builder`
>   → `combined_report_renderer`). The planner-driven path (`report_orchestrator.py`,
>   `report_planner.py`, `config/section_tools.py`) was **removed**.
> - **HTML only** — all PDF rendering (`pdf_renderer.py`, `bureau_pdf_renderer.py`, the
>   `fpdf2` dependency) was **removed**; the only theme is `bank_v2`.
> - **Bureau sub-system removed** — `schemas/bureau_report.py`, bureau extractors/narration,
>   and every `bureau_report` code path are gone. Reports are banking-only.
> - **Layers collapsed** — `features/` (merchant features → `tools/merchant_features.py`)
>   and `pipeline/extractors/` were **removed**; the three category modules now live in the
>   `tools/category/` subpackage.
> - **New canonical homes** — row-level rules in `tools/rules.py`, the review checklist in
>   `pipeline/reports/checklist_builder.py` (stored on `CustomerReport.checklist`),
>   account-quality thresholds in `config/thresholds.py` (`AQ_*`), fuzzy similarity in
>   `utils/narration_utils.are_similar`.
>
> Sections describing bureau analysis, PDF generation, or the section planner are historical.
...
⏺ Suggestions for bank_report_v2.html                                                                                                                            
                                                                                                                                                               
  High-value additions (data already in your pipeline):                                                                                                          
                                                                                                                                                                 
  1. Account Quality badge — tools/account_quality.py classifies CONDUIT/PRIMARY/SECONDARY. Surface as a header pill next to the verdict; it changes how a       
  reviewer reads every other number.                                                                                                                             
  2. Salary regularity panel — replace the approximated "on-time strip" with real per-month detection: bucket actual salary credits by month, show date-of-month 
  variance, flag drift (e.g. 1st → 7th → 12th = employer stress signal).                                                                                         
  3. Day-of-month cashflow heatmap — you already have a 4×7 debit heatmap. Add a credits one stacked beside it; juxtaposition reveals salary→spend lag patterns. 
  4. Counterparty concentration (HHI) — top-5 share of total credits/debits. A CONDUIT account jumps out instantly when one party = 60%+ of inflows.             
  5. Self-transfer / round-trip ledger — event_detector.py already emits round_trip and self_transfer. Dedicated mini-table with paired in/out amounts and the   
  time-gap; this is the #1 fraud-screen item analysts ask for.                                                                                                   
  6. NACH bounce timeline — vertical strip of bounce events with lender + amount; pair with EMI cards so the "3 bounces" number on a card is clickable.          
  7. Negative-balance / min-balance breach days — count of days balance < 0 or < AMB threshold. Single KPI, very high signal.                                    
  8. Inflow vs outflow waterfall (per month) — replace one of the duplicate sparklines with a stacked waterfall: opening → +credits → −debits → −EMIs → closing. 
  More informative than two separate bars.                                                                                                                       
                                                                                                                                                                 
  Smaller polish:                                                                                                                                                
                                                            
  9. Verdict reasoning expandable — the pill says GREEN/AMBER/RED but scorecard.signals[] has the why. Make the pill clickable to reveal the signal list.        
  10. Transaction table chips → filterable — class chips (salary/emi/upi/fraud) exist; wire them to the existing search box as click-to-filter.
  11. Period-over-period delta — KPI cards show current values; add ▲/▼ vs prior 3 months for net cashflow / avg balance / EMI%.                                 
  12. Print-optimised CSS — add @media print rules; banking reports get printed/PDF'd more than viewed in browser.                                               
                                                                                                                                                                 
  New (need small extractor work):                                                                                                                               
                                                                                                                                                                 
  13. Cash deposit / ATM withdrawal ratio — high cash-in-bank ratio is a known underwriting flag; event_detector already tags ATM events.                        
  14. Weekend/late-night transaction ratio — quick anomaly indicator from tran_date parsing alone.
  15. Bureau cross-reference strip — when bureau_report is also available in this theme, show "EMI seen on statement vs. EMI expected from tradelines"           
  reconciliation. Catches off-book loans.                                                                                                                        
                                                                                                                                                                 
  Want me to implement any subset? I'd recommend 1, 4, 5, 7, 9 as the highest-signal-per-effort batch. 
